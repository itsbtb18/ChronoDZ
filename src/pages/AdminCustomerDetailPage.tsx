...
type FilterKey = "ALL" | "EN_ATTENTE" | "PAYE" | "TODAY" | "THIS_WEEK";
...
export function AdminCustomerDetailPage({ language }: AdminCustomerDetailPageProps) {
  ...
  useEffect(() => {
    ...
    const loadCustomer = async () => {
      if (!customerId) { setError("Client introuvable."); setLoading(false); return; }
      try {
        ...
        const bookingResponse = await fetch(`/api/bookings/?search=${encodeURIComponent(payload.phone)}`, { headers: authHeader() });
        if (bookingResponse.ok) {
          const bookingPayload = (await bookingResponse.json()) as Booking[];
          if (active) setBookings(bookingPayload.filter((b) => b.total_price !== undefined));
        }
      } catch (errorValue) {
  ...
  const bookingStats = useMemo(() => ({
    total: bookings.filter((b) => b.status !== "MAINTENANCE").length,
    paid: bookings.filter((b) => b.status === "PAYE").length,
    pending: bookings.filter((b) => b.status === "EN_ATTENTE").length,
    cancelled: bookings.filter((b) => b.status === "ANNULE").length,
  }), [bookings]);
  ...
  const filteredBookings = useMemo(() => {
    ...
    return bookings.filter((b) => {
      ...
      if (activeFilter === "EN_ATTENTE") return b.status === "EN_ATTENTE";
      if (activeFilter === "PAYE") return b.status === "PAYE";
      if (activeFilter === "TODAY") return b.booking_date === today;
      if (activeFilter === "THIS_WEEK") return b.booking_date >= weekStart && b.booking_date <= weekEnd;
      return true;
    });
  ...
  const handleSave = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setSaving(true);
    setError(null);
    if (!firstName.trim() || !lastName.trim() || !phone.trim()) { setError(t("errors.clientFormRequired")); setSaving(false); return; }
    if (!isValidAlgerianPhone(phone)) { setError(t("errors.phoneInvalidFormat")); setSaving(false); return; }
    if (secretCode.trim() && !isValidSecretCode(secretCode)) { setError(t("errors.secretCodeInvalidFormat")); setSaving(false); return; }
    try {
      const payload: Record<string, string> = { first_name: firstName.trim(), last_name: lastName.trim(), phone: normalizePhoneInput(phone) };
      ...
      if (!response.ok) {
        ...
        throw new Error(resolveApiErrorMessage(errorPayload, "adminUpdateCustomer", t, { status: response.status }));
      }
  ...
  const FILTERS: { key: FilterKey; label: string; count?: number }[] = [
    { key: "ALL",        label: "Tous",           count: bookingStats.total },
    { key: "EN_ATTENTE", label: "En attente",      count: bookingStats.pending },
    { key: "PAYE",       label: "Payés",           count: bookingStats.paid },
      ...
      <div className="relative z-10 w-full px-4 sm:px-6 lg:px-8 py-6">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 rounded-[2rem] border border-rose-100 bg-rose-50/50 backdrop-blur-xl shadow-lg text-rose-800 max-w-2xl mx-auto text-center px-6">
            ...
            <button type="button" onClick={() => navigate("/admin/dashboard/creation", { replace: true })} className="mt-6 rounded-xl bg-slate-900 px-5 py-2.5 text-xs font-bold text-white hover:bg-slate-800 transition">
              Retour à la création
            </button>
          </div>
        ) : customer ? (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            {/* Left: Client form (no delete button) */}
            <div className="lg:col-span-5 rounded-[2rem] border border-sky-100/40 bg-white/70 backdrop-blur-xl p-6 sm:p-8 shadow-[0_20px_50px_rgba(15,23,42,0.03)] relative overflow-hidden flex flex-col justify-between">
              ...
              <form onSubmit={handleSave} className="relative z-10 w-full space-y-6">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="rounded-full bg-sky-50 px-2.5 py-0.5 text-[9px] font-black uppercase tracking-[0.18em] text-sky-600 border border-sky-100/50 shrink-0">{customer.role}</div>
                </div>

                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-1.5">
                    <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 ml-1">Nom</label>
                    <input type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder="Nom de famille" required
                      className="w-full rounded-xl border border-slate-200 bg-white/80 px-4 py-3.5 text-sm font-medium text-slate-900 outline-none transition focus:border-sky-400 focus:shadow-[0_0_0_4px_rgba(14,165,233,0.06)]" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 ml-1">Prénom</label>
                    <input type="text" value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder="Prénom" required
                      className="w-full rounded-xl border border-slate-200 bg-white/80 px-4 py-3.5 text-sm font-medium text-slate-900 outline-none transition focus:border-sky-400 focus:shadow-[0_0_0_4px_rgba(14,165,233,0.06)]" />
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input type="text" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="05XX XXX XXX" dir="ltr" required
                      className="w-full rounded-xl border border-slate-200 bg-white py-3.5 pl-11 pr-4 text-sm font-medium text-slate-900 outline-none transition focus:border-sky-400 focus:shadow-[0_0_0_4px_rgba(14,165,233,0.06)]" />
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-3">
                    <div className="relative flex-1">
                      ...
                      <input type="text" value={secretCode} onChange={(e) => setSecretCode(e.target.value)} placeholder="Laisser vide pour ne pas modifier" maxLength={6}
                        className="w-full rounded-xl border border-slate-200 bg-white py-3.5 pl-11 pr-4 tracking-[0.3em] text-sm font-bold text-slate-900 outline-none transition placeholder:tracking-normal focus:border-sky-400 focus:shadow-[0_0_0_4px_rgba(14,165,233,0.06)]" />
                    </div>
                    <button type="button" onClick={() => setSecretCode(randomSecretCode())}
                      className="shrink-0 rounded-xl bg-slate-950 px-5 py-3.5 text-xs font-bold text-white shadow-lg transition hover:bg-slate-800 hover:-translate-y-0.5 active:scale-95 cursor-pointer">
                      Générer
                    </button>
                  </div>
                </div>
                  ...
                  <button type="submit" disabled={saving}
                    ...
                    <span className="relative z-10 flex items-center justify-center gap-2">
                      {saving ? (
                        ...
                        <><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" /></svg>Enregistrer</>
                      )}
                    </span>
            ...
            {/* Right: Stats + filtered bookings list */}
            <div className="lg:col-span-7 space-y-6">
              {/* Info row */}
              <div className="grid gap-4 grid-cols-1 sm:grid-cols-3 rounded-[2rem] border border-sky-100/60 bg-white/85 p-6 shadow-[0_18px_50px_rgba(15,23,42,0.03)] backdrop-blur-xl">
              ...
              <div className="rounded-[2rem] border border-sky-100/40 bg-white/70 backdrop-blur-xl p-6 sm:p-8 shadow-[0_20px_50px_rgba(15,23,42,0.03)] relative overflow-hidden flex flex-col">
                    ...
                    >
                      ...
                      {f.count !== undefined && (
                        <span className={`rounded-full px-1.5 py-0 text-[10px] font-black ${
                          activeFilter === f.key ? "bg-white/25 text-white" : "bg-white text-slate-500 border border-slate-200"
                        }`}>
                          {f.count}
                        </span>
                      )}
                ...
                <div className="max-h-[380px] overflow-y-auto pr-1 space-y-2">
                  {filteredBookings.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-12 text-center rounded-[1.25rem] border border-dashed border-sky-100 bg-slate-50/20">
                      ...
                      <p className="text-xs font-semibold text-slate-500">Aucun rendez-vous pour ce filtre.</p>
                    </div>
                  ) : (
                    filteredBookings.map((booking) => (
                      <button
                        ...
                        className="group flex w-full items-center justify-between gap-4 rounded-2xl border border-sky-100/60 bg-white/90 p-4 text-left shadow-[0_4px_12px_rgba(15,23,42,0.03)] transition hover:-translate-y-px hover:border-sky-200 hover:shadow-[0_12px_28px_rgba(15,23,42,0.06)] cursor-pointer"
                      >
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <span className={`rounded-full px-2.5 py-0.5 text-[9px] font-black uppercase tracking-[0.15em] border ${
                            booking.status === "PAYE" ? "bg-emerald-500/10 text-emerald-600 border-emerald-500/20"
                            : booking.status === "ANNULE" ? "bg-rose-500/10 text-rose-600 border-rose-500/20"
                            : "bg-amber-500/10 text-amber-600 border-amber-500/20"
                          }`}>
                            {booking.status === "PAYE" ? "Payé" : booking.status === "ANNULE" ? "Annulé" : "En attente"}
                          </span>
                          <svg className="w-4 h-4 text-slate-300 transition group-hover:text-sky-500 group-hover:translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
                        </div>
                      </button>
                    ))
                  )}
...
function StatCard({ label, value, tone }: { label: string; value: number; tone: "sky" | "emerald" | "amber" | "rose" }) {
  ...
  return (
    <div className={`rounded-2xl border p-4 transition hover:shadow-md ${toneClasses}`}>
